        ...
        if (mode == -1) {
                ...
                if (mode == 1)
                    ...
                    status--;
                        ...
                        } else if(cm.getPlayer().getMapId() == 180000000) {
                                if (status == 0) {
                                        if (cm.getJobId() == 2000) {
                                                cm.sendOk("你好, 狂狼勇士. 這是很難解釋, 但宇宙是有點...壞了, 請你. 允許我為你解決.");
                                        } else if (cm.getJobId() == 0) {
                                                cm.sendOk("你好, 冒險家. 這裡的設定有點尚未修改, 請你. 允許我為你解決.");
                                        } else if (cm.getJobId() == 1000) {
                                                cm.sendOk("你好, 皇家騎士團. 這裡的設定有點尚未修改, 請你. 允許我為你解決.");
                                        } else {
                                                cm.sendOk("你不應該在這裡!");
                                                cm.dispose();
                                        }
                                        ...
                                        if (cm.getJobId() == 2000) {
                                                ...
                                                cm.sendOk("這些是你的東西. 現在, 讓我們離開這裡.");
                                        } else if (cm.getJobId() == 0) {
                                                cm.sendOk("現在, 為你移送到訓練場.");
                                        } else if (cm.getJobId() == 1000) {
                                                cm.sendOk("現在, 為你移送到訓練場.");
                                        }
                                } else if (status == 2) {
                                        cm.getPlayer().setOfficial(1);
                                        cm.getPlayer().dropMessage(6,"[新生通報]: 恭喜【" + cm.getName() + "】成為正式成員.");
                                        cm.warp(104040000); // to the free market!
                                        cm.dispose();
                                }
                        } else {
                                if (status == 0)
