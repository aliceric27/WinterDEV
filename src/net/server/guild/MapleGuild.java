...
public class MapleGuild {
    private static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MapleGuild.class);// 排行系統
    ...
    public int getIncreaseGuildCost(int size) {
        ...
        public static void showlvl(MapleClient c, int npcid) {
            try {
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement ps = con.prepareStatement("SELECT `name`, `level`, `str`, `dex`, `int`, `luk` FROM characters WHERE gm < 3 ORDER BY `level` DESC LIMIT 100");
                ResultSet rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showGuildRanks3(npcid, rs));
                ps.close();
                rs.close();
            } catch (Exception e) {
                log.error("未能顯示等級排行.", e);
            }
        }
        ...
        public static void showreborns(MapleClient c, int npcid) {
            try {
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement ps = con.prepareStatement("SELECT `name`, `rebirths`, `str`, `dex`, `int`, `luk` FROM characters WHERE gm < 3 ORDER BY `rebirths` DESC LIMIT 50");
                ResultSet rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showGuildRanks5(npcid, rs));
                ps.close();
                rs.close();
            } catch (Exception e) {
                log.error("未能顯示轉升排行.", e);
            }
        }

        public static void showrebornsfs(MapleClient c, int npcid) {
            try {
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement ps = con.prepareStatement("SELECT `name`, `fs`, `str`, `dex`, `int`, `luk` FROM characters WHERE gm < 3 ORDER BY `fs` DESC LIMIT 50");
                ResultSet rs = ps.executeQuery();
                c.getSession().write(MaplePacketCreator.showGuildRanks500(npcid, rs));
                ps.close();
                rs.close();
            } catch (Exception e) {
                log.error("未能顯示飛升排行.", e);
            }
        }
        
        public static void showmeso(MapleClient c, int npcid) {
            try {
                Connection con = DatabaseConnection.getConnection();
                PreparedStatement ps = con.prepareStatement(
                        "SELECT `name`, `meso`, `str`, `dex`, " +
                        "`int`, `luk` FROM characters WHERE gm < 3 ORDER BY `meso` DESC LIMIT 30");
