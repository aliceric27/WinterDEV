...
public class Server implements Runnable {
    ...
    public void run() {
        ...
        try {
            p.load(new FileInputStream("WinterDEV.ini"));
        } catch (Exception e) {
            System.out.println("Please start create_server.bat");
            System.exit(0);
        }
        long loadingStartTime = System.currentTimeMillis();
        System.out.println("WinterDEV v" + ServerConstants.VERSION + " 開機.\r\n");
        System.out.println("IP位置確認："+ ServerConstants.HOST +"\r\n");
        ...
        tMan.register(new RankingWorker(), ServerConstants.RANKING_INTERVAL);
			...
			try {
				System.out.println("載入 commands.jar 指令檔.");
				long startTime = System.currentTimeMillis();
				CommandLoader.getInstance().load(ServerConstants.COMMAND_JAR_PATH);
				System.out.println("    載入完畢 >> 花費 " + ((System.currentTimeMillis() - startTime) / 1000.0) + "秒.\r\n");
			} catch (Exception e) {
				System.out.println("無法載入 commands.jar 指令檔.");
				MapleLogger.print(MapleLogger.EXCEPTION_CAUGHT, e);
			}
		}
        long timeToTake = System.currentTimeMillis();
        System.out.println("加載技能");
        SkillFactory.loadAllSkills();
        System.out.println("    技能加載完畢 >> 花費 " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " 秒\r\n");

        timeToTake = System.currentTimeMillis();
        System.out.println("加載物品");
        MapleItemInformationProvider.getInstance().getAllItems();

        CashItemFactory.getSpecialCashItems();
        System.out.println("    物品加載完畢 >> 花費 " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " 秒\r\n");
        
        try {
            for (int i = 0; i < Integer.parseInt(p.getProperty("worlds")); i++) {
                System.out.println("啟動伺服器 world " + i);
                ...
                System.out.println("加載完成  world " + i + "\r\n");
            }
        } catch (Exception e) {
            System.out.println("Error in WinterDEV.ini, start CreateINI.bat to re-make the file.");
        ...
        System.out.println("服務器已完成連線. (總花費 " + ((System.currentTimeMillis() - loadingStartTime) / 1000.0) + "秒)\r\n");
        online = true;
    }
    ...
    public final Runnable shutdown(final boolean restart) {//only once :D
        return new Runnable() {
            @Override
            public void run() {
                System.out.println((restart ? "重新啟動" : "關閉") + " 伺服器!\r\n");
                ...
                System.out.println("Worlds + Channels 完成關閉.");
                acceptor.unbind();
                acceptor = null;
                if (!restart) {
                    System.exit(0);
