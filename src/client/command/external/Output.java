...
public class Output {
	...
	public static void print(String message, boolean newLine) {
		System.out.print("[WinterDEV] [" + now() + "] " + message + ((newLine) ? "\n" : ""));
		if (ParanoiaConstants.PARANOIA_CONSOLE_LOGGER) {
			MapleLogger.printFormatted(MapleLogger.PARANOIA_CONSOLE, "[WinterDEV] " + message, newLine);
		}
	}
	...
	public static String now() {
		...
		return ((hours + (TimeZone.getDefault().useDaylightTime() ? 0 : -12)) + ":" + ((minutes < 10) ? "0" + minutes : "" + minutes) + ":" + ((seconds < 10) ? "0" + seconds : "" + seconds));
	}
