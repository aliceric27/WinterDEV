...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void maxMastery1() {
	for (MapleData skill_ : MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/" + "String.wz")).getData("Skill.img").getChildren()) {
            try {
                Skill skill = SkillFactory.getSkill(Integer.parseInt(skill_.getName()));
                if (skill.getId() >= 1000000 && skill.getId() <= 5221010 || skill.getId() >= 11000000 && skill.getId() <= 15111007 || skill.getId() >= 21000000 && skill.getId() <= 21121008)
                    getPlayer().changeSkillLevel(skill, (byte) 0, skill.getMaxLevel(), -1);
        } catch (NumberFormatException nfe) {
            break;
        } catch (NullPointerException npe) {
    ...
    public void teachSkills(int[] skillid, int level, int maxlevel) {
        for (MapleData skill_ : MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/" + "String.wz")).getData("Skill.img").getChildren()) {
            try {
                for(int i = 0; i < skillid.length; i++) {
                    Skill skill = SkillFactory.getSkill(Integer.parseInt(skill_.getName()));
                    if(skill.getId() == skillid[i]) {
                        if(level == -1)
                            level = skill.getMaxLevel();
                        if(maxlevel == -1)
                            maxlevel = skill.getMaxLevel();
                        getPlayer().changeSkillLevel(skill, (byte)level, maxlevel, -1);
                    }
                }
            } catch (NumberFormatException nfe) {
                break;
    ...
    public void showrebornsfs() {
        MapleGuild.showrebornsfs(getClient(), npc);
    }

    public void showmeso() {
        MapleGuild.showmeso(getClient(), npc);
    }
